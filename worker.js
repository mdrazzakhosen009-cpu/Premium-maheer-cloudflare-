...
async function getAdminPasswordHash(env, db) {
  const r = await sql(db, 'SELECT value FROM settings WHERE key=?', ['admin_password_hash']);
  if (r.rows[0]?.value) return String(r.rows[0].value);
  return hashPassword(env.ADMIN_PASSWORD || 'admin123');
}
...
async function initDb(env) {
  ...
  await sql(db, 'INSERT OR IGNORE INTO settings(key,value) VALUES(?,?)', ['admin_password_hash', await hashPassword(env.ADMIN_PASSWORD || 'admin123')]);
  return true;
}
...
async function requireAdmin(request, env) {
  ...
  const db = getDb(env);
  if (!db) return false;
  const secret = await getAdminPasswordHash(env, db);
...
async function storeInfo(env) {
  const db = getDb(env);
  const info = { ...defaultSettings };
  if (!db) return info;
  const r = await sql(db, 'SELECT key,value FROM settings');
  r.rows.forEach(x => { info[x.key] = x.value; });
  return info;
}
...
async function chatReply(body, env) {
  ...
  const info = await storeInfo(env);
  const low = msg.toLowerCase();
  const selected = body.contextProduct || findProduct(msg, products);
  const usageQuestion = /how\s*(to|do)\s*(i|you)?\s*use|use|ব্যবহার|ব্যবহার কর|কিভাবে|কীভাবে|কিভাবে ব্যবহার|কি ভাবে ব্যবহার|কাজ করে|কি কাজ|কী কাজ/i.test(msg);
  const productListQuestion = /product|products|পণ্য|প্রোডাক্ট|কি কি আছে|কী কী আছে|কী কী পণ্য|কি কি পণ্য|দেখাও|show/i.test(low);
  const storeQuestion = /store info|about store|store|shop|দোকান|স্টোর|শপ|ঠিকানা|address|যোগাযোগ|contact|phone|ফোন|whatsapp|ইমেইল|email/i.test(low);

  if (env.GEMINI_API_KEY && !/\border\b|অর্ডার|কিনতে|buy/i.test(msg)) {
    try {
      const model = env.GEMINI_MODEL || 'gemini-2.5-flash';
      const compact = products.slice(0, 50).map(p => `${p.name} | ৳${p.price} | ${p.category} | stock ${p.stock} | rating ${p.rating} (${p.reviews}) | admin_usage: ${p.usage || 'Not set'} | description: ${p.description || ''}`).join('\n');
      const context = 'You are MAHEER Store Assistant for a Bangladesh beauty and personal-care ecommerce store. Reply concisely in the customer language. Use only supplied facts. IMPORTANT: product usage instructions must come from the product admin_usage field. Never invent a usage routine, price, stock, delivery time, phone number, address or policy. If admin_usage is missing, say the usage instruction is not available yet and suggest asking the store/reading the product label. You can show products, explain prices/ratings, explain store info, help place orders, and track orders.\nStore info: ' + JSON.stringify(info) + '\nCatalog:\n' + compact + '\nCurrent product context: ' + JSON.stringify(selected || null);
  ...
  if (/^(hello|hi|assalam|হাই|হ্যালো)/.test(low)) return { reply: 'Welcome to MAHEER STORE ✨ Product, price, usage, delivery, store info বা order নিয়ে আমাকে জিজ্ঞেস করতে পারেন।' };
  ...
  if (/\border\b|অর্ডার|কিনতে|buy/.test(low)) {
    ...
    return selected
      ...
      : { reply: 'অবশ্যই ❤️ কোন productটি কিনতে চান? Product name লিখুন বা Shop থেকে product খুলে আমাকে “order” বলুন।' };
  }
  if (storeQuestion) {
    return { reply: `MAHEER STORE INFO:\n${info.about || ''}\n📞 Phone: ${info.phone || 'Not set'}\n💬 WhatsApp: ${info.whatsapp || 'Not set'}\n✉️ Email: ${info.email || 'Not set'}`, };
  }
  if (productListQuestion) {
    const list = products.slice(0, 10).map((p, i) => `${i + 1}. ${p.name} — ৳${Number(p.price).toLocaleString('en-BD')} — ${p.category}`).join('\n');
    return { reply: list ? `আমাদের available products:\n${list}\n\nযেকোনো product-এর নাম লিখলে আমি price, rating, stock বা admin-set usage বলব।` : 'এই মুহূর্তে কোনো product পাওয়া যাচ্ছে না।', product: selected || undefined };
  }
  ...
  if (usageQuestion) {
    if (!selected) return { reply: 'কোন product-এর ব্যবহার জানতে চান? Product name লিখুন, অথবা Shop থেকে product খুলে তারপর “কিভাবে ব্যবহার করবো?” বলুন।' };
    return selected.usage
      ? { reply: `${selected.name} কীভাবে ব্যবহার করবেন:\n${selected.usage}`, product: selected }
      : { reply: `${selected.name}-এর জন্য Admin এখনো usage instruction সেট করেনি। Product label অনুযায়ী ব্যবহার করুন বা store agent-এর সাথে যোগাযোগ করুন।`, product: selected };
  }
  ...
  if (/delivery|ডেলিভারি|shipping|সময়|সময়|কতদিন|কয়দিন|কত দিন|কয় দিনে/.test(low)) return { reply: 'MAHEER STORE সারা বাংলাদেশে delivery support করে। Exact delivery time location ও courier-এর ওপর নির্ভর করতে পারে। Checkout-এ ঠিকানা দিয়ে order confirm করুন।' };
  return selected
    ? { reply: `${selected.name} — ৳${Number(selected.price).toLocaleString('en-BD')}, ${selected.rating}★ rating, ${selected.reviews} reviews.${selected.usage ? `\nUsage: ${selected.usage}` : ''}`, product: selected }
    : { reply: 'আমি products দেখাতে, price/rating/stock বলতে, Admin-set usage জানাতে, store info দিতে, delivery সম্পর্কে বলতে, order নিতে এবং order track করতে সাহায্য করতে পারি।' };
}
...
async function handleApi(request, env, ctx) {
  ...
  try {
    if ((env.TURSO_DATABASE_URL || env.TURSO_URL) && (env.TURSO_AUTH_TOKEN || env.TURSO_TOKEN)) await ensureDb(env);
    ...
    if (method === 'POST' && path === '/api/admin/login') {
      ...
      const secret = await getAdminPasswordHash(env, db);
      const incoming = await hashPassword(body.password || '');
      if (incoming !== secret) return json({ error: 'Wrong password' }, 401);
    ...
    if (method === 'GET' && path === '/api/reviews') {
      const db = getDb(env);
      if (!db) return json([]);
      const r = await sql(db, 'SELECT r.id,r.product_id,r.name,r.rating,r.comment,r.created_at,p.name AS product_name FROM reviews r LEFT JOIN products p ON p.id=r.product_id WHERE r.approved=1 ORDER BY r.id DESC');
      return json(r.rows);
    }
    ...
    if (path.startsWith('/api/admin/')) {
      ...
      if (method === 'PUT' && path === '/api/admin/password') {
        ...
        const currentHash = await getAdminPasswordHash(env, db);
        if ((await hashPassword(currentPassword)) !== currentHash) return json({ error: 'Current password is incorrect.' }, 401);
        const newHash = await hashPassword(newPassword);
        await sql(db, 'INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value', ['admin_password_hash', newHash]);
        return json({ ok: true, message: 'Password changed successfully. Please log in again.' });
      }

      if (method === 'DELETE' && /^\/api\/admin\/reviews\/\d+$/.test(path)) {
        const id = Number(path.split('/').pop());
        if (!Number.isInteger(id) || id < 1) return json({ error: 'Invalid review id.' }, 400);
        const old = await sql(db, 'SELECT product_id FROM reviews WHERE id=?', [id]);
        if (!old.rows.length) return json({ error: 'Review not found.' }, 404);
        const productId = Number(old.rows[0].product_id);
        await sql(db, 'DELETE FROM reviews WHERE id=?', [id]);
        const remaining = await sql(db, 'SELECT ROUND(AVG(rating),1) AS avg, COUNT(*) AS count FROM reviews WHERE product_id=? AND approved=1', [productId]);
        const avg = remaining.rows[0]?.avg == null ? 4.8 : Number(remaining.rows[0].avg);
        const count = Number(remaining.rows[0]?.count || 0);
        await sql(db, 'UPDATE products SET rating=?,reviews=? WHERE id=?', [avg, count, productId]);
        return json({ ok: true });
      }
